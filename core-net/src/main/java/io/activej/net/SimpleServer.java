 ...
 * each {@link TcpSocket acync client connection} to a given consumer.
 */
public final class SimpleServer extends AbstractReactiveServer<SimpleServer> {
	private final Consumer<TcpSocket> socketConsumer;

	private SimpleServer(NioReactor reactor, Consumer<TcpSocket> socketConsumer) {
		super(reactor);
		this.socketConsumer = socketConsumer;
	}

	public static SimpleServer create(NioReactor reactor, Consumer<TcpSocket> socketConsumer) {
		return new SimpleServer(reactor, socketConsumer);
	}

	public static SimpleServer create(Consumer<TcpSocket> socketConsumer) {

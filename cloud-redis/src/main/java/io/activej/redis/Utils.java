...
final class Utils {
	...
	static <T> BiFunction<T, @Nullable Throwable, Promise<? extends T>> wrapException(Supplier<String> errorMessageSupplier) {
		return (v, e) -> e == null ?
				Promise.of(v) :
				e instanceof RedisException ?
						Promise.ofException(e) :
						Promise.ofException(new RedisException(errorMessageSupplier.get(), e));
	}
}

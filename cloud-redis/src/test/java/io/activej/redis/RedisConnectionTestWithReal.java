...
import static io.activej.redis.ScanModifier.match;
import static io.activej.redis.SortModifier.alpha;
import static io.activej.redis.SortModifier.desc;
	...
	public void watch() {
		...
		List<Object> execResult = io.activej.redis.TestUtils.await(client, redis -> redis.watch(key1, key2)
				...
				.then(() -> {
					...
					return redis.exec()
							.whenResult(() -> assertThat(setPromise.getException(), instanceOf(TransactionFailedException.class)));
				}));

...
public final class SerializerBuilder2 {
	private static final Path DEFAULT_SAVE_DIR = getPathSetting(SerializerBuilder2.class, "saveDir", null);
	...
	public SerializerBuilder2(DefiningClassLoader classLoader) {
		this.classLoader = classLoader;
	}

	public static SerializerBuilder2 create(DefiningClassLoader definingClassLoader) {
		SerializerBuilder2 builder = new SerializerBuilder2(definingClassLoader);
		...
		builder
				...
				.with(Long.class, ctx -> new SerializerDefLong(true, ctx.hasAnnotation(SerializeVarLength.class)))
		...
		builder
				...
				.with(String.class, ctx -> new SerializerDefString(
						ctx.getAnnotation(S2StringFormat.class, a -> a == null ? StringFormat.UTF8 : a.value())))
	...
	public SerializerBuilder2 with(TypeT<?> typeT, MappingFunction<SerializerDef, Cache> fn) {
		return with(typeT.getType(), fn);
	}

	@SuppressWarnings("PointlessBooleanExpression")
	public SerializerBuilder2 with(Type type, MappingFunction<SerializerDef, Cache> fn) {
		registry.with(type, ctx -> {
			Class<?> rawClass = ctx.getRawClass();
			SerializerDef serializerDef;
			S2Class annotationClass;
			S2Subclasses annotationSubclass;
			if (false ||
					(annotationClass = ctx.getAnnotation(S2Class.class)) != null ||
					(annotationClass = getAnnotation(S2Class.class, rawClass.getAnnotations())) != null) {
				try {
					...
					(annotationSubclass = ctx.getAnnotation(S2Subclasses.class)) != null ||
					(annotationSubclass = getAnnotation(S2Subclasses.class, rawClass.getAnnotations())) != null) {
			...
			if (ctx.hasAnnotation(S2Reference.class) ||
					hasAnnotation(S2Reference.class, rawClass.getAnnotations())) {
				serializerDef = new SerializerDefReference(serializerDef);
			}

			if (ctx.hasAnnotation(S2VarLength.class)) {
				serializerDef = ((SerializerDefWithVarLength) serializerDef).ensureVarLength();
			}

			S2FixedSize annotationFixedSize;
			if ((annotationFixedSize = ctx.getAnnotation(S2FixedSize.class)) != null) {
				serializerDef = ((SerializerDefWithFixedSize) serializerDef).ensureFixedSize(annotationFixedSize.value());
			}

			if (ctx.hasAnnotation(S2Nullable.class)) {
	...
	public SerializerBuilder2 withSubclasses(String extraSubclassesId, Class<?>... subclasses) {
		return withSubclasses(extraSubclassesId, Arrays.asList(subclasses));
	}

	public SerializerBuilder2 withSubclasses(String subclassesId, List<Class<?>> subclasses) {
		extraSubclassesMap.put(subclassesId, subclasses);
		return this;
	}

	public SerializerBuilder2 withCompatibilityLevel(CompatibilityLevel compatibilityLevel) {
	 ...
	 */
	public SerializerBuilder2 withGeneratedBytecodePath(Path path) {
		this.saveBytecodePath = path;
		return this;
	}

	public SerializerBuilder2 withEncodeVersion(int encodeVersionMax) {
		this.encodeVersionMax = encodeVersionMax;
		return this;
	}

	public SerializerBuilder2 withDecodeVersions(int decodeVersionMin, int decodeVersionMax) {
		this.decodeVersionMin = decodeVersionMin;
		this.decodeVersionMax = decodeVersionMax;
		return this;
	}

	public SerializerBuilder2 withVersions(int encodeVersionMax, int decodeVersionMin, int decodeVersionMax) {
		this.encodeVersionMax = encodeVersionMax;
		this.decodeVersionMin = decodeVersionMin;
		this.decodeVersionMax = decodeVersionMax;
		return this;
	...
	public SerializerBuilder2 withProfile(String profile) {
		this.profile = profile;
		return this;
	}

	public SerializerBuilder2 withClassKey(Object... classKeyParameters) {
	...
	public <T> SerializerBuilder2 withSubclasses(Class<T> type, List<Class<? extends T>> subclasses) {
	...
	private static SerializerDef wrapNullable(Context<SerializerDef, Cache> ctx, SerializerDef serializerDef) {
		return hasAnnotation(S2Nullable.class, ctx.getAnnotations()) ?
				new SerializerDefNullable(serializerDef) :
				serializerDef;
	}
	...
	public <T> BinarySerializer<T> build(AnnotatedType type) {
		//noinspection unchecked
		return (BinarySerializer<T>) buildImpl(registry.scanner(new Cache()).scan(type));
	}
	...
	private Expression encoderImpl(ClassBuilder<?> classBuilder, SerializerDef serializer, @Nullable Integer encodeVersion, SerializerDef.StaticEncoders staticEncoders, Expression buf, Variable pos, Expression data) {
		return sequence(
				encodeVersion != null ?
	...
	private Expression decodeImpl(ClassBuilder<?> classBuilder, SerializerDef serializer, Integer latestVersion, SerializerDef.StaticDecoders staticDecoders,
			Expression in) {
	...
	private SerializerDef.StaticDecoders staticDecoders(ClassBuilder<?> classBuilder) {
	...
	private void scanFactories(Context<SerializerDef, Cache> ctx, Function<TypeVariable<?>, AnnotatedType> bindings,
			SerializerDefClass serializer) {
		DeserializeFactory annotationFactory = ctx.getAnnotation(DeserializeFactory.class);
		Class<?> factoryClassType = (annotationFactory == null) ? ctx.getRawClass() : annotationFactory.value();
	...
	private FoundSerializer tryAddField(Context<SerializerDef, Cache> ctx, Function<TypeVariable<?>, AnnotatedType> bindings,
			Field field) {
		SerializerBuilder2.FoundSerializer result = findAnnotations(ctx, field, field.getAnnotations());
		if (result == null) {
			return null;
		}

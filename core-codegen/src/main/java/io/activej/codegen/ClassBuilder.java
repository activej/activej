...
import io.activej.codegen.expression.impl.Expression_Constant;
...
public final class ClassBuilder<T> {
	...
	final Map<String, Expression_Constant> fieldConstants = new HashMap<>();
	...
	public final class Builder extends AbstractBuilder<Builder, ClassBuilder<T>> {
		...
		public Builder withStaticFinalField(String field, Class<?> type, Expression value) {
			...
			fieldsFinal.add(field);
			fieldExpressions.put(field, value);
			if (value instanceof Expression_Constant constantExpression) {
	...
	public GeneratedBytecode toBytecode(ClassLoader classLoader, String className) {
...

		{
			...
			for (Map.Entry<String, Expression_Constant> entry : this.fieldConstants.entrySet()) {
		...
		return new GeneratedBytecode(className, bytecode) {
			...
			private void cleanup() {
				for (Map.Entry<String, Expression_Constant> entry : fieldConstants.entrySet()) {

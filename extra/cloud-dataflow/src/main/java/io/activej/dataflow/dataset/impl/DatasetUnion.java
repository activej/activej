...
public final class DatasetUnion<T> extends Dataset<T> {
	private final Dataset<T> left;
	private final Dataset<T> right;

	public DatasetUnion(Dataset<T> left, Dataset<T> right) {
...
//		Map<Partition, List<StreamId>> partitioned = new HashMap<>(graph.getAvailablePartitions().size());
//		doPartition(graph, leftStreamIds, partitioned);
//		doPartition(graph, rightStreamIds, partitioned);
	...
	private static void doPartition(DataflowGraph graph, List<StreamId> leftStreamIds, Map<Partition, List<StreamId>> partitioned) {
		for (StreamId leftStreamId : leftStreamIds) {
			partitioned.computeIfAbsent(graph.getPartition(leftStreamId), $ -> new ArrayList<>()).add(leftStreamId);
		}
	}
}

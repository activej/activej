...
public final class DatasetOffsetLimit<K, T> extends SortedDataset<K, T> {
	...
	public List<StreamId> channels(DataflowContext context) {
		...
		List<StreamId> newStreamIds = DatasetUtils.repartitionAndReduce(next, streamIds, valueType(), input.keyFunction(), input.keyComparator(), mergeReducer(), singletonList(randomPartition));
	...
	private List<StreamId> toOutput(DataflowGraph graph, int index, StreamId streamId) {
		...
		return singletonList(node.getOutput());
	}
}

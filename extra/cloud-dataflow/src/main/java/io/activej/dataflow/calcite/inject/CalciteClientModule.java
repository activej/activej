...
public final class CalciteClientModule extends AbstractModule {
	...
	@Provides
	CalciteSqlDataflow calciteSqlDataflow(NioReactor reactor, DataflowClient client, SqlParser.Config parserConfig, SqlToRelConverter sqlToRelConverter, RelOptPlanner planner,
			List<Partition> partitions, RelToDatasetConverter relToDatasetConverter) {
		return CalciteSqlDataflow.create(reactor, client, partitions, parserConfig, sqlToRelConverter, planner, relToDatasetConverter);
	}
}

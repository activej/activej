...
import io.activej.cube.Cube_Reactive.AggregationContainer;
...
public class TestCompatibleAggregations {
	...
	private Cube_Reactive cubeWithDetailedAggregation;

	@Before
	public void setUp() {
		cube = new Cube_Reactive(null, null, null, null)
		...
		cubeWithDetailedAggregation = new Cube_Reactive(null, null, null, null)
				.withInitializer(cube -> {
					MEASURES.forEach(cube::addMeasure);
					DIMENSIONS_DAILY_AGGREGATION.forEach(cube::addDimension);
					DIMENSIONS_ADVERTISERS_AGGREGATION.forEach(cube::addDimension);
	...
	public void withWherePredicateAlwaysTrue_MatchesDailyAggregation() {
		...
		Aggregation_Reactive expected = cube.getAggregation(DAILY_AGGREGATION.getId());
	...
	public void withWherePredicateForAdvertisersAggregation_MatchesAdvertisersAggregation() {
		...
		Aggregation_Reactive expected = cube.getAggregation(ADVERTISERS_AGGREGATION.getId());
	...
	public void withWherePredicateForAffiliatesAggregation_MatchesAffiliatesAggregation() {
		...
		Aggregation_Reactive expected = cube.getAggregation(AFFILIATES_AGGREGATION.getId());
	...
	public void withWherePredicateForBothAffiliatesAggregations_MatchesAffiliatesAggregation() {
		...
		Aggregation_Reactive expected = cubeWithDetailedAggregation.getAggregation(DETAILED_AFFILIATES_AGGREGATION.getId());
	...
	public void withWherePredicateForDailyAggregation_MatchesOnlyDailyAggregations() {
		...
		Aggregation_Reactive expected = cubeWithDetailedAggregation.getAggregation(DAILY_AGGREGATION.getId());
		Aggregation_Reactive expected2 = cubeWithDetailedAggregation.getAggregation(LIMITED_DATES_AGGREGATION.getId());
	...
	public void withWherePredicateForAdvertisersAggregation_MatchesOneAggregation() {
		...
		Aggregation_Reactive expected = cubeWithDetailedAggregation.getAggregation(ADVERTISERS_AGGREGATION.getId());

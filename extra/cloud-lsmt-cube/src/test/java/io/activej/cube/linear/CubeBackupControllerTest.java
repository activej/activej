	...
	public void backup() throws CubeException {
		List<LogDiff<CubeDiff>> diffs1 = singletonList(
				LogDiff.of(
						...
						CubeDiff.of(map(
								"pub", AggregationDiff.of(set(chunk(12), chunk(123), chunk(500))),
								"adv", AggregationDiff.of(set(chunk(10), chunk(1), chunk(44)))
						))
				));
		...
		List<LogDiff<CubeDiff>> diffs2 = singletonList(
				LogDiff.of(
						...
						CubeDiff.of(map(
								"pub", AggregationDiff.of(set(chunk(43)), set(chunk(500))),
								"adv", AggregationDiff.of(set(chunk(512), chunk(786)), set(chunk(44)))
						))
				));
		...
		List<LogDiff<CubeDiff>> diffs3 = singletonList(
				LogDiff.of(
						...
						CubeDiff.of(map(
								"pub", AggregationDiff.of(set(chunk(4566)), set(chunk(12)))
						))
				));
		...
		assertChunkIds(1, set(1L, 10L, 12L, 44L, 123L, 500L));
		assertPositions(1, map(
				"a", LogPosition.create(new LogFile("a", 12), 100),
				"b", LogPosition.create(new LogFile("b", 1123), 1000)
		));

		assertChunkIds(2, set(1L, 10L, 12L, 43L, 123L, 512L, 786L));
		assertPositions(2, map(
				"a", LogPosition.create(new LogFile("a", 12), 100),
				"b", LogPosition.create(new LogFile("b2", 9), 2341),
				"c", LogPosition.create(new LogFile("c", 555), 12)
		));

		assertChunkIds(3, set(1L, 10L, 43L, 123L, 512L, 786L, 4566L));
		assertPositions(3, map(
				"a", LogPosition.create(new LogFile("a", 12), 100),
				"b", LogPosition.create(new LogFile("b2", 9), 2341),
				"c", LogPosition.create(new LogFile("c", 555), 12),

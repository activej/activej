...
public final class CrdtStorage_Fs<K extends Comparable<K>, S> extends AbstractReactive
		implements AsyncCrdtStorage<K, S>, WithInitializer<CrdtStorage_Fs<K, S>>, ReactiveService, ReactiveJmxBeanWithStats {
	private static final Logger logger = LoggerFactory.getLogger(CrdtStorage_Fs.class);

	public static final Duration DEFAULT_SMOOTHING_WINDOW = ApplicationSettings.getDuration(CrdtStorage_Fs.class, "smoothingWindow", Duration.ofMinutes(1));
	...
	// region creators
	private CrdtStorage_Fs(Reactor reactor, AsyncFs fs, BinarySerializer_CrdtData<K, S> serializer, CrdtFunction<S> function) {
		super(reactor);
		this.fs = fs;
	...
	public static <K extends Comparable<K>, S> CrdtStorage_Fs<K, S> create(
			Reactor reactor, AsyncFs fs,
			BinarySerializer_CrdtData<K, S> serializer,
			CrdtFunction<S> function
	) {
		return new CrdtStorage_Fs<>(reactor, fs, serializer, function);
	}

	public static <K extends Comparable<K>, S extends CrdtType<S>> CrdtStorage_Fs<K, S> create(
			Reactor reactor, AsyncFs fs,
			BinarySerializer_CrdtData<K, S> serializer
	) {
		return new CrdtStorage_Fs<>(reactor, fs, serializer, CrdtFunction.ofCrdtType());
	}

	public CrdtStorage_Fs<K, S> withNamingStrategy(Supplier<String> namingStrategy) {
		this.namingStrategy = namingStrategy;
		return this;
	}

	public CrdtStorage_Fs<K, S> withFilter(CrdtFilter<S> filter) {
		this.filter = filter;
		return this;
	}
	// endregion
	...
	private Promise<Void> doConsolidate() {
		return fs.list("*")
				...
				.map(CrdtStorage_Fs::pickFilesForConsolidation)

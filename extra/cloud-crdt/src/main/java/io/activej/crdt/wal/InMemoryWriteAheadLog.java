...
public class InMemoryWriteAheadLog<K extends Comparable<K>, S> extends AbstractReactive
		implements AsyncWriteAheadLog<K, S>, ReactiveService, WithInitializer<InMemoryWriteAheadLog<K, S>> {
	...
	private InMemoryWriteAheadLog(Reactor reactor, CrdtFunction<S> function, AsyncCrdtStorage<K, S> storage) {
		super(reactor);
		this.function = function;
		this.storage = storage;
	}

	public static <K extends Comparable<K>, S> InMemoryWriteAheadLog<K, S> create(Reactor reactor, CrdtFunction<S> function, AsyncCrdtStorage<K, S> storage) {
		return new InMemoryWriteAheadLog<>(reactor, function, storage);
	}

	public static <K extends Comparable<K>, S extends CrdtType<S>> InMemoryWriteAheadLog<K, S> create(Reactor reactor, AsyncCrdtStorage<K, S> storage) {

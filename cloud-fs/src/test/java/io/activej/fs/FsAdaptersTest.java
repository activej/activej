...
public final class FsAdaptersTest {
	...
	private AsyncFs local;

	@Before
	public void setup() throws IOException {
		Path path = temporaryFolder.newFolder("test").toPath();
		Fs localFs = Fs.create(getCurrentReactor(), newSingleThreadExecutor(), path);
		await(localFs.start());
		this.local = localFs;
	}

	private void upload(AsyncFs fs, String filename) {
		await(fs.upload(filename).then(ChannelConsumer::acceptEndOfStream));
	}

	private void uploadForbidden(AsyncFs fs, String filename) {
	...
	public void addingPrefix() {
		AsyncFs prefixed = FsAdapters.addPrefix(local, "prefix/");
	...
	public void strippingPrefix() {
		AsyncFs prefixed = FsAdapters.removePrefix(local, "prefix/");
	...
	public void mountingClient() {
		AsyncFs root = FsAdapters.subdirectory(local, "root");
		AsyncFs first = FsAdapters.subdirectory(local, "first");
	...
	public void filterClient() {
		AsyncFs filtered = FsAdapters.filter(local, s -> s.endsWith(".txt") && Pattern.compile("\\d{2}").matcher(s).find());

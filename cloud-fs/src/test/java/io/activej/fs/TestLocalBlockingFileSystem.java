...
import static io.activej.fs.LocalBlockingFileSystem.DEFAULT_TEMP_DIR;
...
public final class TestLocalBlockingFileSystem {
	...
	private LocalBlockingFileSystem client;
	...
	public void setup() throws IOException {
		...
		client = LocalBlockingFileSystem.create(storagePath);
		client.start();
	}
	...
	public void testUploadToSameNewDir() throws IOException {
		...
		Set<String> filenames = IntStream.range(0, 5)
				.mapToObj(i -> dir + BlockingFileSystem.SEPARATOR + i + ".txt")
				.collect(toSet());
		...
		Map<String, FileMetadata> files = client.list(dir + BlockingFileSystem.SEPARATOR + '*');
	...
	public void testRelativePaths() throws IOException {
		...
		client = LocalBlockingFileSystem.create(relativePath);
		client.start();

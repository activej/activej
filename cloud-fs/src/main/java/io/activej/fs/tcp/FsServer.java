...
/**
 * An implementation of {@link AbstractReactiveServer} that works with {@link RemoteFs} client.
 * It exposes some given {@link AsyncFs} via TCP.
 * <p>
...
public final class FsServer extends AbstractReactiveServer<FsServer> {
	...
	private FsServer(NioReactor reactor, AsyncFs fs) {
		super(reactor);
		this.fs = fs;
	}

	public static FsServer create(NioReactor reactor, AsyncFs fs) {
		return new FsServer(reactor, fs);
	}

	public FsServer withHandshakeHandler(Function<FsRequest.Handshake, FsResponse.Handshake> handshakeHandler) {
		this.handshakeHandler = handshakeHandler;
		return this;
	}

	public AsyncFs getFs() {
		return fs;
	}

	@Override
	protected void serve(AsyncTcpSocket socket, InetAddress remoteAddress) {
		ReactiveMessaging<FsRequest, FsResponse> messaging =
	...
	private Promise<Void> dispatch(ReactiveMessaging<FsRequest, FsResponse> messaging, FsRequest msg) throws Exception {
	...
	private Promise<Void> handleHandshake(AsyncMessaging<FsRequest, FsResponse> messaging, FsRequest.Handshake
			handshake) {
	...
	private Promise<Void> handleUpload(AsyncMessaging<FsRequest, FsResponse> messaging, FsRequest.Upload upload) {
	...
	private Promise<Void> handleAppend(AsyncMessaging<FsRequest, FsResponse> messaging, FsRequest.Append append) {
	...
	private Promise<Void> handleDownload(AsyncMessaging<FsRequest, FsResponse> messaging, FsRequest.Download download) {
	...
	private Promise<Void> handleCopy(AsyncMessaging<FsRequest, FsResponse> messaging, FsRequest.Copy copy) throws
			Exception {
		return simpleHandle(messaging, () -> fs.copy(copy.name(), copy.target()), FsResponse.CopyFinished::new, copyPromise);
	}

	private Promise<Void> handleCopyAll(AsyncMessaging<FsRequest, FsResponse> messaging, FsRequest.CopyAll copyAll) throws
			Exception {
		return simpleHandle(messaging, () -> fs.copyAll(copyAll.sourceToTarget()), FsResponse.CopyAllFinished::new, copyAllPromise);
	}

	private Promise<Void> handleMove(AsyncMessaging<FsRequest, FsResponse> messaging, FsRequest.Move move) throws
			Exception {
		return simpleHandle(messaging, () -> fs.move(move.name(), move.target()), FsResponse.MoveFinished::new, movePromise);
	}

	private Promise<Void> handleMoveAll(AsyncMessaging<FsRequest, FsResponse> messaging, FsRequest.MoveAll moveAll) throws
			Exception {
		return simpleHandle(messaging, () -> fs.moveAll(moveAll.sourceToTarget()), FsResponse.MoveAllFinished::new, moveAllPromise);
	}

	private Promise<Void> handleDelete(AsyncMessaging<FsRequest, FsResponse> messaging, FsRequest.Delete delete) throws
			Exception {
		return simpleHandle(messaging, () -> fs.delete(delete.name()), FsResponse.DeleteFinished::new, deletePromise);
	}

	private Promise<Void> handleDeleteAll(AsyncMessaging<FsRequest, FsResponse> messaging, FsRequest.DeleteAll
	...
	private Promise<Void> handleList(AsyncMessaging<FsRequest, FsResponse> messaging, FsRequest.List list) throws
			Exception {
		return simpleHandle(messaging, () -> fs.list(list.glob()), FsResponse.ListFinished::new, listPromise);
	}

	private Promise<Void> handleInfo(AsyncMessaging<FsRequest, FsResponse> messaging, FsRequest.Info info) throws
			Exception {
		return simpleHandle(messaging, () -> fs.info(info.name()), FsResponse.InfoFinished::new, infoPromise);
	}

	private Promise<Void> handleInfoAll(AsyncMessaging<FsRequest, FsResponse> messaging, FsRequest.InfoAll infoAll) throws
			Exception {
		return simpleHandle(messaging, () -> fs.infoAll(infoAll.names()), FsResponse.InfoAllFinished::new, infoAllPromise);
	}

	private Promise<Void> handlePing(AsyncMessaging<FsRequest, FsResponse> messaging) throws Exception {
		return simpleHandle(messaging, fs::ping, FsResponse.Pong::new, pingPromise);
	}

	private Promise<Void> simpleHandle(
			AsyncMessaging<FsRequest, FsResponse> messaging,

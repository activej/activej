 ...
 */
public final class RemoteFs extends AbstractNioReactive
		implements AsyncFs, ReactiveService, ReactiveJmxBeanWithStats, WithInitializer<RemoteFs> {
	private static final Logger logger = LoggerFactory.getLogger(RemoteFs.class);
	...
	// region creators
	private RemoteFs(NioReactor reactor, InetSocketAddress address) {
		super(reactor);
		this.address = address;
	}

	public static RemoteFs create(NioReactor reactor, InetSocketAddress address) {
		return new RemoteFs(reactor, address);
	}

	public RemoteFs withSocketSettings(SocketSettings socketSettings) {
		this.socketSettings = socketSettings;
		return this;
	}

	public RemoteFs withConnectionTimeout(Duration connectionTimeout) {
	...
	private Promise<ReactiveMessaging<FsResponse, FsRequest>> connect(InetSocketAddress address) {
		return doConnect(address, socketSettings);
	}

	private Promise<ReactiveMessaging<FsResponse, FsRequest>> connectForStreaming(InetSocketAddress address) {
		return doConnect(address, socketSettings.withLingerTimeout(Duration.ZERO));
	}

	private Promise<ReactiveMessaging<FsResponse, FsRequest>> doConnect(InetSocketAddress address, SocketSettings socketSettings) {

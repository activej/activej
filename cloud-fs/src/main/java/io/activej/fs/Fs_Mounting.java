...
/**
 * A file system that allows to mount several {@link AsyncFs} implementations to correspond to different filenames.
 * <p>
 * Inherits the most strict limitations of all the mounted file systems implementations and root file system.
 */
final class Fs_Mounting implements AsyncFs {
	private final AsyncFs root;
	private final Map<String, AsyncFs> mounts;

	Fs_Mounting(AsyncFs root, Map<String, AsyncFs> mounts) {
		this.root = root;
		this.mounts = mounts;
	}

	private AsyncFs findMount(String filename) {
		int idx = filename.lastIndexOf('/');
		while (idx != -1) {
			String path = filename.substring(0, idx);
			AsyncFs mount = mounts.get(path);
			if (mount != null) {
				return mount;
			}
	...
	public Promise<Void> copyAll(Map<String, String> sourceToTarget) {
		...
		return transfer(sourceToTarget, AsyncFs::copyAll, false);
	}
	...
	public Promise<Void> moveAll(Map<String, String> sourceToTarget) {
		...
		return transfer(sourceToTarget, AsyncFs::moveAll, true);
	}
	...
	private Promise<Void> transfer(String source, String target, BiFunction<String, String, AsyncConsumer<AsyncFs>> action, boolean deleteSource) {
		AsyncFs first = findMount(source);
		AsyncFs second = findMount(target);
		if (first == second) {
	...
	private Promise<Void> transfer(Map<String, String> sourceToTarget, AsyncBiConsumer<AsyncFs, Map<String, String>> action, boolean deleteSource) {
		List<AsyncSupplier<Tuple2<String, Try<Void>>>> movePromises = new ArrayList<>();

		Map<AsyncFs, Map<String, String>> groupedBySameFs = new IdentityHashMap<>();
		...
		for (Map.Entry<AsyncFs, Map<String, String>> entry : groupedBySameFs.entrySet()) {
					...
					if (!exceptions.isEmpty()) {
						...
						for (Tuple2<String, Exception> tuple : exceptions) {
							...
							if (exception instanceof FsScalarException) {
								scalarExceptions.put(tuple.value1(), (FsScalarException) exception);

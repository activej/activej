...
public final class ClusterFileSystem extends AbstractReactive
	...
	private ClusterFileSystem(Reactor reactor, FileSystemPartitions partitions) {
		super(reactor);
		this.partitions = partitions;
	}

	public static ClusterFileSystem create(Reactor reactor, FileSystemPartitions partitions) {
		return builder(reactor, partitions).build();
	}

	public static Builder builder(Reactor reactor, FileSystemPartitions partitions) {
		return new ClusterFileSystem(reactor, partitions).new Builder();
	}
	...
	private Promise<Void> ensureIsAlive() {
		return isAlive() ?
			Promise.complete() :
			Promise.ofException(new FileSystemIOException(
				"There are more dead partitions than allowed(" +

...
/**
 * A service that allows to discover actual {@link AsyncFs} partitions in a cluster
 */
public interface AsyncDiscoveryService {

	/**
	 * Discovers actual {@link AsyncFs} partitions in a cluster
	 * <p>
	 ...
	 * <p>
	 */
	AsyncSupplier<Map<Object, AsyncFs>> discover();

	/**
	 * A {@code DiscoveryService} that consists of given partitions that never change
	 *
	 * @param partitions constant {@link AsyncFs} partitions
	 * @return a constant discovery service
	 */
	static AsyncDiscoveryService constant(Map<Object, AsyncFs> partitions) {
		Map<Object, AsyncFs> constant = Map.copyOf(partitions);
		return () -> new AsyncSupplier<>() {
			int i = 0;

			@Override
			public Promise<Map<Object, AsyncFs>> get() {

 ...
 * </ul>
 */
public final class Fs_Cluster extends AbstractReactive
		implements AsyncFs, WithInitializer<Fs_Cluster>, ReactiveService, ReactiveJmxBeanWithStats {
	private static final Logger logger = LoggerFactory.getLogger(Fs_Cluster.class);
	...
	// region creators
	private Fs_Cluster(Reactor reactor, FsPartitions partitions) {
		super(reactor);
		this.partitions = partitions;
	}

	public static Fs_Cluster create(Reactor reactor, FsPartitions partitions) {
		return new Fs_Cluster(reactor, partitions);
	}

	/**
	 * Sets the replication count that determines how many copies of the file should persist over the cluster.
	 */
	public Fs_Cluster withReplicationCount(int replicationCount) {
	...
	public Fs_Cluster withPersistenceOptions(int deadPartitionsThreshold, int uploadTargetsMin, int uploadTargetsMax) {

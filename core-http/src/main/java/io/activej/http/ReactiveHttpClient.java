...
public final class ReactiveHttpClient extends AbstractNioReactive
	...
	public static final Duration CONNECT_TIMEOUT = ApplicationSettings.getDuration(ReactiveHttpClient.class, "connectTimeout", Duration.ZERO);
	public static final Duration READ_WRITE_TIMEOUT = ApplicationSettings.getDuration(ReactiveHttpClient.class, "readWriteTimeout", Duration.ZERO);
	public static final Duration READ_WRITE_TIMEOUT_SHUTDOWN = ApplicationSettings.getDuration(ReactiveHttpClient.class, "readWriteTimeout_Shutdown", Duration.ofSeconds(3));
	public static final Duration KEEP_ALIVE_TIMEOUT = ApplicationSettings.getDuration(ReactiveHttpClient.class, "keepAliveTimeout", Duration.ZERO);
	public static final MemSize MAX_BODY_SIZE = ApplicationSettings.getMemSize(ReactiveHttpClient.class, "maxBodySize", MemSize.ZERO);
	...
	private @Nullable ReactiveTcpSocket.Inspector socketInspector;
	private @Nullable ReactiveTcpSocket.Inspector socketSslInspector;
	...
	public ReactiveHttpClient withNoKeepAlive() {
		return withKeepAliveTimeout(Duration.ZERO);
	}

	public ReactiveHttpClient withMaxKeepAliveRequests(int maxKeepAliveRequests) {
	...
	public ReactiveHttpClient withMaxBodySize(MemSize maxBodySize) {
		return withMaxBodySize(maxBodySize.toInt());
	}

	public ReactiveHttpClient withMaxBodySize(int maxBodySize) {
		this.maxBodySize = maxBodySize != 0 ? maxBodySize : Integer.MAX_VALUE;
		return this;
	}

	public ReactiveHttpClient withMaxWebSocketMessageSize(MemSize maxWebSocketMessageSize) {
		this.maxWebSocketMessageSize = maxWebSocketMessageSize.toInt();
		return this;
	}

	public ReactiveHttpClient withInspector(Inspector inspector) {
		this.inspector = inspector;
		return this;
	}

	public ReactiveHttpClient withSocketInspector(ReactiveTcpSocket.Inspector socketInspector) {
		this.socketInspector = socketInspector;
		return this;
	}

	public ReactiveHttpClient withSocketSslInspector(ReactiveTcpSocket.Inspector socketSslInspector) {
	...
	private Promise<?> doSend(HttpRequest request, InetAddress[] inetAddresses, boolean isWebSocket) {
		...
		return ReactiveTcpSocket.connect(reactor, address, connectTimeoutMillis, socketSettings)
				.then(
						asyncTcpSocketImpl -> {
							ReactiveTcpSocket.Inspector socketInspector = isSecure ? this.socketInspector : socketSslInspector;
	...
	@JmxAttribute
	public @Nullable ReactiveTcpSocket.JmxInspector getSocketStats() {
		return BaseInspector.lookup(socketInspector, ReactiveTcpSocket.JmxInspector.class);
	}

	@JmxAttribute
	public @Nullable ReactiveTcpSocket.JmxInspector getSocketStatsSsl() {
		return BaseInspector.lookup(socketSslInspector, ReactiveTcpSocket.JmxInspector.class);
	}

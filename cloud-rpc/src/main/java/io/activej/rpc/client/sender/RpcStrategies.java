...
public final class RpcStrategies {
	...
	public static RpcStrategy_SingleServer server(InetSocketAddress address) {
		return RpcStrategy_SingleServer.create(address);
	}
	...
	public static List<RpcStrategy> servers(List<InetSocketAddress> addresses) {
		...
		return addresses.stream()
				.map(RpcStrategy_SingleServer::create)
				.collect(toList());
	}
